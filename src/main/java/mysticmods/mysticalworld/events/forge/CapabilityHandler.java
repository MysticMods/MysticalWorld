...
public class CapabilityHandler {
  ...
  public static void onPlayerJoin(PlayerEvent.PlayerLoggedInEvent event) {
    ...
    player.getCapability(Capabilities.PLAYER_SHOULDER).ifPresent((cap) -> {
      if (cap.isShouldered()) {
        ShoulderRide message = new ShoulderRide(event.getEntity(), cap);
        ...
        if (!event.getLevel().isClientSide) {
          event.getTarget().getCapability(Capabilities.ANIMAL_COOLDOWN).ifPresent(cap -> {
            if (cap.hasExpired()) {
              ...
              event.getLevel().playLocalSound(player.getX(), player.getY(), player.getZ(), ModSounds.SQUID_MILK.get(), SoundSource.PLAYERS, 0.5F, event.getLevel().random.nextFloat() * 0.25F + 0.6F, true);
              if (!player.isCreative()) {
                heldItem.shrink(1);
              }
              player.getInventory().add(new ItemStack(ModItems.INK_BOTTLE.get()));
            } else {
              player.displayClientMessage(Component.translatable("message.squid.cooldown").setStyle(Style.EMPTY.withColor(TextColor.fromLegacyFormat(ChatFormatting.BLUE)).withBold(true)), true);
            }
          });
        }
      }
