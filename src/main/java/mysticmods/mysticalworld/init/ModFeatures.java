...
public class ModFeatures {
  ...
  private static final RegistryObject<ConfiguredFeature<?, ?>> CONFIGURED_STONEPETAL_PATCH = CONFIGURED_FEATURES.register("stonepetal_patch", () -> new ConfiguredFeature<>(Feature.RANDOM_PATCH, new RandomPatchConfiguration(ConfigManager.STONEPETAL_CONFIG.getTries(), 3, 4, STONEPETAL.getHolder().get())));
  ...
  private static final RegistryObject<ConfiguredFeature<?, ?>> CONFIGURED_WILD_AUBERGINE_PATCH = CONFIGURED_FEATURES.register("wild_aubergine_patch", () -> new ConfiguredFeature<>(Feature.RANDOM_PATCH, new RandomPatchConfiguration(ConfigManager.WILD_AUBERGINE.getTries(), 4, 2, WILD_AUBERGINE.getHolder().get())));

  public static final RegistryObject<PlacedFeature> WILD_AUBERGINE_PATCH = PLACED_FEATURES.register("wild_aubergine_patch", () -> new PlacedFeature(CONFIGURED_WILD_AUBERGINE_PATCH.getHolder().get(), List.of(CountPlacement.of(ConfigManager.WILD_AUBERGINE.getRepeats()), InSquarePlacement.spread(), RarityFilter.onAverageOnceEvery(90), DimensionPlacement.of(ConfigManager.WILD_AUBERGINE.getDimensions()), PlacementUtils.HEIGHTMAP, BiomeFilter.biome())));
