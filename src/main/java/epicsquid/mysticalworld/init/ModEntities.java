  ...
  public static void registerEntity(BiomeLoadingEvent event) {
    for (Map.Entry<RegistryEntry<? extends EntityType<?>>, MobConfig> entry : configMap.entrySet()) {
      ...
      if (conf.shouldRegister()) {
        if (conf.getBiomes().contains(event.getCategory())) {
          event.getSpawns().getSpawner(conf.isMob() ? EntityClassification.MONSTER : EntityClassification.CREATURE).add(new MobSpawnInfo.Spawners(entry.getKey().get(), conf.getChance(), conf.getMin(), conf.getMax()));
        }
      }
    }
  }
