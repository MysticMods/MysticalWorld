...
public class SpindleStringRecipe extends ShapelessOreRecipe {
  public SpindleStringRecipe(ResourceLocation group, NonNullList<Ingredient> input, @Nonnull ItemStack result) {
  ...
  public NonNullList<ItemStack> getRemainingItems(InventoryCrafting inv) {
    ...
    for (int i = 0; i < result.size(); i++) {
      ...
      if (IngredientSpindle.getInstance().apply(inSlot)) {
        inSlot = inSlot.copy();
  ...
  public static class Factory implements IRecipeFactory {
    ...
    public IRecipe parse(JsonContext context, JsonObject json) {
      ...
      return new SpindleStringRecipe(group.isEmpty() ? null : new ResourceLocation(group), ings, itemstack);
    }
  }
}
