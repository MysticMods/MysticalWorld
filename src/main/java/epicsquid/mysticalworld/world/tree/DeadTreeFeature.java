package epicsquid.mysticalworld.world.tree;
...
public class DeadTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
  private static final Supplier<BlockState> CHARRED_LOG = () -> ModBlocks.CHARRED_LOG.get().getDefaultState();
  private static final Supplier<BlockState> LEAVES = Blocks.AIR::getDefaultState;

  public DeadTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> configFactoryIn, boolean doBlockNotifyIn) {
      ...
      for (int k = -i; k <= i; ++k) {
        if (Math.pow((double) Math.abs(j) + 0.5D, 2.0D) + Math.pow((double) Math.abs(k) + 0.5D, 2.0D) <= (double) (p_208529_3_ * p_208529_3_)) {
          ...
          if (isAirOrLeaves(worldIn, blockpos)) {
            this.setLogState(changedBlocks, worldIn, blockpos, LEAVES.get(), p_208529_4_);
          }
        }
      }
    }
  ...
  private int makeLimb(Set<BlockPos> p_208523_1_, IWorldGenerationReader worldIn, BlockPos p_208523_3_, BlockPos p_208523_4_, boolean p_208523_5_, MutableBoundingBox p_208523_6_) {
    if (!p_208523_5_ && Objects.equals(p_208523_3_, p_208523_4_)) {
      ...
      for (int j = 0; j <= i; ++j) {
        ...
        if (p_208523_5_) {
          this.setLogState(p_208523_1_, worldIn, blockpos1, CHARRED_LOG.get().with(LogBlock.AXIS, this.getLoxAxis(p_208523_3_, blockpos1)), p_208523_6_);
